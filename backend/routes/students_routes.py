...
@router.post("/add")
def addStudent(student: Student, db_connection: Session = Depends(get_db)):
    return student_func.addStudent(student, db_connection)

@router.put("/edit/{student_id}")
def editStudent(student: Student, student_id: str, db_connection: Session = Depends(get_db)):
    return student_func.editStudent(student, student_id, db_connection)

@router.delete("/delete/{student_id}")
def deleteStudent(student_id: str, db_connection: Session = Depends(get_db)):
